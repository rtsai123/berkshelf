module Berkshelf
  class Berksfile
    ...
    # Determines if the Berksfile is "ok". A Berksfile is ok if all the dependencies
    # defined in the Berksfile are also stored in the lockfile AND all the dependencies
    # are downloaded to the local file system.
    #
    # @return [Boolean]
    def ok?
      dependencies.select { |dependency| lockfile.find(dependency.name).nil? }.empty? &&
      lockfile.dependencies.select { |dependency| !dependency.downloaded? }.empty?
    end
